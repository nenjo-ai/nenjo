...
use nenjo::knowledge::{KnowledgeDocEdgeType, KnowledgePack};
use nenjo::knowledge_tools::{
...
use crate::knowledge_backend::{
    ...
    is_current_project_pack_selector, is_nenjo_pack_selector, parse_project_pack_selector,
    project_pack_selector, unknown_pack,
};
...
pub struct PlatformManifestBackend<L, E> {
    ...
    current_project_slug: Option<String>,
}
...
impl<L, E> PlatformManifestBackend<L, E> {
    ...
    /// Attach the active project slug used to resolve the `project` knowledge pack alias.
    pub fn with_current_project_slug(mut self, project_slug: Option<String>) -> Self {
        self.current_project_slug = project_slug.filter(|slug| !slug.trim().is_empty());
        self
    }
    ...
    E: SensitivePayloadEncoder,
{
    ...
    async fn project_workspace_dir(&self, project_slug: &str) -> Result<PathBuf> {
        let project = self.project_by_slug(project_slug).await?;
        Ok(self.workspace_dir()?.join(project.slug))
    }

    async fn project_by_slug(&self, project_slug: &str) -> Result<ProjectManifest> {
        self.local_store
            .get_project_by_slug(project_slug)
            .await?
            .ok_or_else(|| anyhow!("project '{project_slug}' is not cached locally"))
    }

    async fn project_knowledge_pack(&self, project_slug: &str) -> Result<ProjectKnowledgePack> {
        let project_dir = self.project_workspace_dir(project_slug).await?;
        ProjectKnowledgePack::load(&project_dir).ok_or_else(|| {
            anyhow!("project documents are not cached locally for project '{project_slug}'")
        })
    }
    ...
    async fn resolve_knowledge_pack(&self, selector: &str) -> Result<ResolvedKnowledgePack> {
        ...
        if is_current_project_pack_selector(selector) {
            let project_slug = self.current_project_slug.as_deref().ok_or_else(|| {
                anyhow!(
                    "knowledge pack 'project' requires an active project; use project:<slug> outside project context"
                )
            })?;
            return self
                .project_knowledge_pack(project_slug)
                .await
                .map(ResolvedKnowledgePack::Project);
        }
        if selector.starts_with("project:") {
            let project_slug = parse_project_pack_selector(selector)?;
            return self
                .project_knowledge_pack(project_slug)
                .await
                .map(ResolvedKnowledgePack::Project);
    ...
    E: SensitivePayloadEncoder + Send + Sync,
{
    async fn list_packs(&self) -> Result<Vec<KnowledgePackSummary>> {
        ...
        if self.workspace_dir.is_some() {
            for project in self.local_store.list_projects().await? {
                let selector = project_pack_selector(&project.slug);
                let project_dir = self.workspace_dir()?.join(&project.slug);
                if let Some(pack) = ProjectKnowledgePack::load(project_dir) {
                    if self.current_project_slug.as_deref() == Some(project.slug.as_str()) {
                        packs.push(KnowledgePackSummary::new("project", pack.manifest()));
                    }
    ...
    async fn resolve_pack(&self, selector: &str) -> Result<Box<dyn KnowledgePack>> {
        self.resolve_knowledge_pack(selector)
            .await
            .map(|pack| Box::new(pack) as Box<dyn KnowledgePack>)
    }
}
...
mod tests {
    ...
    async fn project_backend_fixture() -> Result<(
        ...
        Ok((backend, project_id, project_slug.to_string(), temp))
    }

    #[tokio::test]
    async fn project_knowledge_alias_resolves_active_project_pack() {
        let (backend, project_id, project_slug, _temp) = project_backend_fixture().await.unwrap();
        let backend = backend.with_current_project_slug(Some(project_slug));

        let packs = backend.list_knowledge_packs().await.unwrap();
        let packs = packs.as_array().expect("packs array");
        assert!(packs.iter().any(|pack| pack["pack"] == "project"));
        ...
        assert_eq!(value["pack"], "project");
        assert_eq!(
            value["virtual_path"],
            format!("project://{project_id}/docs/routine.md")
        );
    }
    ...
    async fn project_document_neighbors_expose_outgoing_and_incoming_edges() {
        ...
        let value = backend
            .list_knowledge_neighbors(json!({
                "pack": format!("project:{project_slug}"),
                "path": "routine"
            }))
            .await
            .unwrap();
    ...
    async fn local_manifest_org_id_uses_cached_bootstrap_org_id() {
        let (backend, _project_id, _project_slug, _temp) = project_backend_fixture().await.unwrap();
