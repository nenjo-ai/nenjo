...
impl GitHubFetcher {
    ...
    async fn resolve_git_ref(&self, namespace: &str) -> Result<Option<String>> {
        ...
        let object = value
            ...
            .ok_or_else(|| PackageError::fetch("GitHub ref response missing object"))?;
        ...
        object
            ...
            .map(str::to_string)
            .map(Some)
            .ok_or_else(|| PackageError::fetch("GitHub ref response missing object.sha"))
    }
...
fn github_get(
    ...
    let mut request = client.get(url).header("User-Agent", "nenjo-packages");
