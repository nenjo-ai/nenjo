...
pub(crate) struct DelegateToToolParams<P = ErasedProvider> {
    pub provider: P,
    ...
    P: ProviderRuntime,
{
    ...
    fn parameters_schema(&self) -> serde_json::Value {
        ...
        let agent_names: Vec<&str> = manifest
            .agents
            .iter()
    ...
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResult> {
        ...
        let target_agent = match self.provider.find_agent_manifest(&agent_name) {
            Some(a) => a,
            None => {
                let manifest = self.provider.manifest_snapshot();
        ...
        let mut builder = match self.provider.build_agent_by_name(&agent_name).await {
            Ok(b) => b,
            Err(e) => {
                return Ok(ToolResult {
                    success: false,
        ...
        debug!(
            ...
            runner
                .instance()
                .build_prompts(&crate::input::AgentRun::chat(crate::input::ChatInput {
                    message: task.clone(),
                    history: vec![],
                    project_id: None,
                }))
