//! Reliability wrapper providing exponential-backoff retries, provider fallback,
...
mod tests {
    ...
    async fn succeeds_without_retry() {
        ...
        let result = one_shot(&provider, None, "hello", "test", 0.0)
            .await
            .unwrap();
        assert_eq!(result, "ok");
    ...
    async fn returns_aggregated_error_when_all_providers_fail() {
        ...
        let err = one_shot(&provider, None, "hello", "test", 0.0)
            .await
    ...
    async fn model_failover_tries_fallback_model() {
        ...
        let result = one_shot(&provider, None, "hello", "claude-opus", 0.0)
            .await
            .unwrap();
    ...
    async fn model_failover_all_models_fail() {
        ...
        let err = one_shot(&provider, None, "hello", "model-a", 0.0)
            .await
    ...
    #[async_trait]
    impl ModelProvider for Arc<ModelAwareMock> {
        async fn chat(
            &self,
