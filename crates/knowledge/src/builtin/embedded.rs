...
pub const NENJO_KNOWLEDGE_DISCOVERY: &str = "Nenjo knowledge is available as the built-in Nenjo knowledge pack. Use list_knowledge_tree, search_knowledge_paths, read_knowledge_doc_manifest, list_knowledge_neighbors, and read_knowledge_doc with pack=\"builtin:nenjo\" when platform concepts or built-in patterns are relevant.";
...
pub fn nenjo_pack() -> &'static BuiltinKnowledgePack {
    PACK.get_or_init(|| {
...
pub fn nenjo_pack_summary() -> String {
    let pack = nenjo_pack();
    let ctx = BuiltinKnowledgeSummaryContext {
        source: "builtin",
        name: "nenjo",
        root: "builtin://nenjo/",
        usage: NENJO_KNOWLEDGE_DISCOVERY,
        docs: pack
            .manifest
            .docs
            .iter()
