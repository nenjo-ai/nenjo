...
pub struct Config {
    ...
    /// Optional user-defined worker name shown in the platform UI.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub harness_name: Option<String>,

    /// Optional user-defined worker labels shown in the platform UI.
...
pub struct WebConfig {
    /// Allow web tools to reach private/local hosts when explicitly listed in
...
pub struct HttpRequestConfig {
    ...
    #[serde(default)]
    pub enabled: bool,
    /// Allowed hosts for HTTP requests (exact/subdomain match; optional `:port`)
    #[serde(default)]
    pub allowed_hosts: Vec<String>,
...
pub struct SessionConfig {
    /// Enable cleanup of file-backed terminal session data.
    #[serde(default = "default_true")]
    pub cleanup_enabled: bool,
    /// Retain terminal file-backed sessions for this many days before cleanup.
    #[serde(default = "default_session_retention_days")]
    pub retention_days: u64,
    /// Run cleanup once when the worker starts.
    #[serde(default = "default_true")]
...
mod tests {
    ...
    #[test]
    fn session_config_deserializes_user_retention_settings() {
