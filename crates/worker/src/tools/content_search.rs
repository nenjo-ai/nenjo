...
impl Tool for ContentSearchTool {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        let mut cmd = if self.has_rg {
            build_rg_command(&RgOptions {
                pattern,
                search_path: &resolved_canon,
                output_mode,
                include,
                case_sensitive,
                context_before,
...
fn build_rg_command(opts: &RgOptions<'_>) -> std::process::Command {
    let &RgOptions {
        pattern,
        search_path: _,
        output_mode,
