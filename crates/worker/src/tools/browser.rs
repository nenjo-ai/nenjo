...
impl BrowserTool {
    ...
    pub fn new_with_config(security: Arc<SecurityPolicy>, config: BrowserToolConfig) -> Self {
        Self {
            security,
            allowed_hosts: normalize_hosts(config.allowed_hosts),
    ...
    fn validate_url(&self, url: &str) -> anyhow::Result<()> {
        ...
        if self.allowed_hosts.is_empty() {
            anyhow::bail!(
                "Browser tool enabled but no allowed_hosts configured. \
                Add [browser].allowed_hosts in config.toml"
            );
        }
        ...
        let private_host = is_private_host(host);
        if private_host && !self.allow_private_hosts {
        ...
        if private_host && !matches_explicit_allowed_host {
            anyhow::bail!(
        ...
        if !matches_allowed_host {
            anyhow::bail!("Host '{host}' not in browser.allowed_hosts");
        }
...
impl Tool for BrowserTool {
    ...
    fn description(&self) -> &str {
        concat!(
            ...
            "interactive elements to refs (@e1, @e2). Enforces browser.allowed_hosts for open actions."
        )
    }
...
fn extract_host(url_str: &str) -> anyhow::Result<TargetHost> {
    ...
    let host = if authority.starts_with('[') {
        ...
        let host = authority.find(']').map_or(authority, |i| &authority[..=i]);
...
fn host_matches_allowlist(target: &TargetHost, allowed: &[String], include_wildcard: bool) -> bool {
    allowed.iter().any(|pattern| {
        if pattern == "*" {
            return include_wildcard;
        }

        let Ok((pattern, pattern_port)) = split_host_port(pattern) else {
            return false;
        };

        if pattern_port.is_some() && pattern_port != target.port {
            return false;
        }

        if let Some(wildcard_host) = pattern.strip_prefix("*.") {
            // Wildcard subdomain match
            let suffix = &pattern[1..]; // ".example.com"
            target.host.ends_with(suffix) || target.host == wildcard_host
        } else {
            // Exact match or subdomain
            target.host == pattern || target.host.ends_with(&format!(".{pattern}"))
        }
    })
}
...
mod tests {
    ...
    #[test]
    fn normalize_hosts_works() {
        let hosts = vec![
            "  Example.COM  ".into(),
            "docs.example.com".into(),
            String::new(),
        ];
        let normalized = normalize_hosts(hosts);
    ...
    fn extract_host_handles_ipv6() {
        ...
        assert_eq!(extract_host("https://[::1]/path").unwrap().host, "[::1]");
        // IPv6 with brackets and port
        assert_eq!(
            extract_host("https://[2001:db8::1]:8080/path")
                .unwrap()
                .host,
            "[2001:db8::1]"
        );
        ...
        assert_eq!(
            extract_host("https://[fe80::1]/").unwrap().host,
            "[fe80::1]"
        );
    }
    ...
    fn validate_url_blocks_ipv6_ssrf() {
        ...
        let tool = BrowserTool::new(security, vec!["*".into()], false, None);
    ...
    #[test]
    fn host_matches_allowlist_honors_ports() {
        let allowed = vec!["example.com:8443".into()];
        assert!(host_matches_allowlist(
            &TargetHost {
                host: "example.com".into(),
                port: Some(8443)
    ...
    fn browser_tool_default_backend_is_agent_browser() {
        let security = Arc::new(SecurityPolicy::default());
        let tool = BrowserTool::new(security, vec!["example.com".into()], false, None);
        assert_eq!(
    ...
    #[test]
    fn browser_tool_private_hosts_require_global_setting_and_explicit_host() {
        let security = Arc::new(SecurityPolicy::default());
        let wildcard = BrowserTool::new(security.clone(), vec!["*".into()], true, None);
        let err = wildcard.validate_url("https://localhost:8080").unwrap_err();
        assert!(err.to_string().contains("explicitly listed"));

        let explicit = BrowserTool::new(security, vec!["localhost:8080".into()], true, None);
    ...
    fn browser_tool_empty_allowlist_blocks() {
        let security = Arc::new(SecurityPolicy::default());
        let tool = BrowserTool::new(security, vec![], false, None);
