...
pub fn turn_event_to_stream_events(
    ...
    match event {
        ...
        nenjo::TurnEvent::ResourceCapacityWaiting { pool, limit } => {
            vec![StreamEvent::ProgressUpdate {
                run_id: run_id.to_string(),
                status: "waiting_for_capacity".into(),
                message: format!("Waiting for {pool} capacity (limit {limit})."),
            }]
        }
        nenjo::TurnEvent::ResourceCapacityAcquired { pool } => vec![StreamEvent::ProgressUpdate {
            run_id: run_id.to_string(),
            status: "capacity_acquired".into(),
            message: format!("Acquired {pool} capacity."),
        }],
...
pub fn summarize_turn_event(event: &nenjo::TurnEvent) -> String {
    match event {
        ...
        nenjo::TurnEvent::ResourceCapacityWaiting { pool, limit } => {
            format!("capacity_waiting(pool={pool}, limit={limit})")
        }
        nenjo::TurnEvent::ResourceCapacityAcquired { pool } => {
            format!("capacity_acquired(pool={pool})")
        }
